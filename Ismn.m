function [ Is_entry ] = Ismn(KER, Tf, Ts, mg, ng, mj, nj, wf, ws, Nf, Ns, doextract)
  % Allocate some memory.
  Is_s = zeros(Nf, 3);
  % Proper treatment of the self terms.
  if mg == ng
    Is_entry = 0.25 / pi .* (Tf.getSelfTermIs(mj, nj) ...
      -1i .* KER.k .* Tf.getArea .* Tf.getSelfTermIn(mj, nj));
...
%        
%     Is_entry = ProjectionIntegral(wf, ...
%                                   Nf, ...
%                                   Tf.getArea, ...
%                                   Tf.getF(KER.rf, mj), ...
%                                   Is_s);
